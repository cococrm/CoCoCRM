    ...
    $.project = (function () {
        return {
            ...
            addToAccordion: function (id, title, icon, select, url) {
                ...
                $("#" + id).accordion('add', {
                    title: title,
                    content: $.project.addToNavTree(url),
                    selected: select
                });
            },
            //加载导航tree
            addToNavTree: function (url) {
                return '<ul class="easyui-tree" data-options="url:\'' + url + '\',method:\'get\',onClick:$.project.clickMainMenuNode"></ul>';
            },
            //单击左侧菜单树
